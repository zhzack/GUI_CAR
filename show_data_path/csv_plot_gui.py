...
class CSVPlotterApp:
    def __init__(self, master):
        ...
        # 文件选择按钮
        self.load_button = tk.Button(master, text="选择CSV文件", command=self.load_csv)
        ...
        # 绘图按钮
        self.plot_button = tk.Button(master, text="绘图", command=self.plot_selected_columns, state=tk.DISABLED)
    ...
    def load_csv(self):
        ...
        # X轴单选
        tk.Label(self.scrollable_frame, text="选择X轴：", font=("Arial", 10, "bold")).grid(row=0, column=0, sticky="w", columnspan=3)
        ...
        tk.Label(self.scrollable_frame, text="选择Y轴（可多选）：", font=("Arial", 10, "bold")).grid(row=start_row, column=0, sticky="w", columnspan=3)
        for idx, col in enumerate(columns):
            var = tk.BooleanVar()
            chk = tk.Checkbutton(self.scrollable_frame, text=col, variable=var)
            chk.grid(row=start_row + 1 + idx // 3, column=idx % 3, sticky="w", padx=5, pady=2)
    ...
    def plot_selected_columns(self):
        ...
        try:
            start_idx = int(self.start_entry.get()) if self.start_entry.get().strip() else 0
            end_idx = int(self.end_entry.get()) if self.end_entry.get().strip() else len(self.df)
        except ValueError:
        ...
        try:
            x_values = list(range(len(sub_df))) if x_col == '' else sub_df[x_col].tolist()
            for c in curves:
